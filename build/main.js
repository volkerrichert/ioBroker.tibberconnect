...
class Tibberconnect extends utils.Adapter {
    ...
    async get_extrem_values(namespaceWithHomeId, LastEnd) {
        if (!luxon_1.DateTime.fromISO(LastEnd).isValid) {
            const ErrorMsg = "Entry provided for LastEnd is no valid formatted Date (expect ISO 8601 string). LastEnd: " + LastEnd;
            await this.setStateAsync(namespaceWithHomeId + ".Calculations.Feedback", {
                val: "Error: " + ErrorMsg,
                ack: true,
            });
